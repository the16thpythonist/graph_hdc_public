...
PyG InMemoryDataset for ZINC molecular graphs from SMILES.

Node features: [atom_type, degree-1, formal_charge, total_Hs, is_in_ring]
- Atom types: 9 (Br, C, Cl, F, I, N, O, P, S)
- Degrees: 6 values (0-5)
...
ZINC_IDX_TO_ATOM: dict[int, str] = {v: k for k, v in ZINC_ATOM_TO_IDX.items()}
...
def mol_to_data(mol: Chem.Mol) -> Data:
    """Convert RDKit molecule to PyG Data with ZINC features."""
    x = []
    for atom in mol.GetAtoms():
        sym = atom.GetSymbol()
        if sym not in ZINC_ATOM_TO_IDX:
            raise ValueError(f"Unexpected atom '{sym}' for ZINC.")
        x.append([
...
class ZincSmiles(InMemoryDataset):
    ...
    def __init__(
        self,
        root: str | Path = DATASET_PATH / "ZincSmiles",
        split: str = "train",
    ...
    def process(self):
        ...
        for s in tqdm(iter_smiles(src), desc=f"ZincSmiles[{self.split}]"):
